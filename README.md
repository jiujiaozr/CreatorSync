...
本次第六次迭代按“真实平台接入预研与半自动发布版”落地。它不承诺四个平台都能真实一键发布，而是先把真实接入前必须看清的限制、权限和人工发布步骤做成可操作界面。

已增加内容：

- 工作台顶部更新为 `CreatorSync V6`，说明当前是半自动发布清单版。
- 发布前检查增强：除了必填字段，还检查标题长度、正文长度、标签数量、公众号摘要、小红书封面标题和 B站脚本摘要。
- 预览发布区新增半自动发布工具：复制当前平台发布内容、打开目标平台发布入口、生成发布前清单。
- 接入预研页新增平台授权与限制表，展示授权状态、所需权限、真实 API 难度、审核或风控限制，以及下一步建议。
- 第七次迭代优先方向明确为“微信公众号草稿箱同步”，先做真实草稿小闭环，不直接做群发。

本次仍然不做：

- 不接入真实平台发布 API。
- 不保存公众号、知乎、B站、小红书账号密码。
- 不保存平台 access token。
- 不绕过平台审核、身份认证、内容风控或发布频率限制。

平台判断说明：

- B站开放平台文档入口是 `https://openhome.bilibili.com/doc`，有稿件、专栏等能力，但需要开放平台账号、应用接入和身份认证。
- 知乎数据开放平台入口是 `https://developer.zhihu.com/`，当前更适合做搜索、问答、热榜等数据能力预研，不优先作为内容写入试点。
- 小红书开放平台入口是 `https://open.xiaohongshu.com/`，公开能力更偏商家和电商服务场景，笔记发布权限和风控要求较重。
- 微信公众号草稿箱适合第七次迭代试点，因为当前项目主要处理图文内容；但它仍然需要后端保管 AppSecret、处理素材上传，并遵守公众号平台规则。
